...
for(qw(/Berg_efter_land /) {
#	push @categories, GetRecursiveCategoryTree("Berg_efter_land");
	push @categories, GetRecursiveCategoryTree("Vattenfall_efter_land");
	push @categories, GetRecursiveCategoryTree("Vattendrag_efter_land");
	push @categories, GetRecursiveCategoryTree("Öar_efter_land");
	push @categories, GetRecursiveCategoryTree("Halvöar_efter_land");
	push @categories, GetRecursiveCategoryTree("Stränder_efter_land");
	push @categories, GetRecursiveCategoryTree("Dalar_efter_land");
	
}
foreach my $p (sort @categories) {
	print "$p\n";
}
